...
7. 在「素材库」上传图片或通用文件，按文件名搜索或筛选图片/文件，切换网格／列表视图，重命名、下载原件，并查看引用素材的事项（点击可跳转，含已完成和未设日期事项）。素材列表不显示 MIME 类型；图片列表和事项附件小图读取私有的压缩 WebP 缩略图，点开大图和打印时读取原图。普通文件限 20 MiB，可预览图片限 20 MB（图片转换服务的输入上限）；PNG、JPEG、GIF、WebP 图片支持预览，其余格式仅作为文件下载。事项编辑弹窗的「附件」可直接上传并添加，或从素材库选择已有文件；每个事项最多关联 50 个素材。素材与事项的关联在保存事项时生效；取消编辑不会删除已上传至素材库的文件。被事项引用的素材不能从素材库删除，请先在所有事项中移除关联。
