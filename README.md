...
7. 在「素材库」上传图片或通用文件，按文件名搜索或筛选图片/文件，查看引用次数、预览图片和下载原件。单文件限 20 MiB；PNG、JPEG、GIF、WebP 图片支持预览，其余格式仅作为文件下载。事项编辑弹窗的「附件」可直接上传并添加，或从素材库选择已有文件；每个事项最多关联 50 个素材。事项卡片展示附件，图片可点开大图；单事项打印页也会展示图片和附件名称。素材与事项的关联在保存事项时生效；取消编辑不会删除已上传至素材库的文件。被事项引用的素材不能从素材库删除，请先在所有事项中移除关联。
...
- `0005_assets.sql` 增加素材元数据、事项附件关联及 R2 删除重试表；复合外键限制邮箱，素材在被事项引用时不可删除。文件内容位于 R2，数据库仅存元数据和对象键；删除时先以 D1 批处理移除元数据并记录待清理键，再删除 R2 文件，失败可在下次上传/删除时重试。

## Cloudflare 部署

部署结构：**一个 Cloudflare Worker（Static Assets 前端 + `/api/*`）→ D1 + 私有 R2**。页面、JS/CSS 和接口共用一个域名、一次部署；不再需要 Pages 项目。下面的命令会创建或更新远程资源。
