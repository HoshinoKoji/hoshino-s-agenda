...
2. 在 header 面包屑中切换「项目日历 / 项目总览」。总览按项目展示所有日期的事项（含空项目），默认隐藏已完成事项及全部完成的分组。顶部显示「未完成 / 已完成」数量，点击「已完成」及其数字切换显隐，展开时以粉色高亮；统计数量不随显隐变化。事项先按目标日期、再按添加时间从早到晚排列，未设目标日期的事项排在最前。总览及日历详情的事项卡片显示本地时区的添加日期；左侧项目筛选对两种视图均有效。切换时保留当前筛选、已完成显隐状态、日历日期和月 / 日视图，刷新默认回到月日历并隐藏已完成。
