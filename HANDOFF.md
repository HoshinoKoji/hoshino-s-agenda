...
- 素材网格卡片改用纵向伸展布局，内容区填满整张卡片，使下载、重命名、删除按钮始终对齐卡片底边；列表沿用内容区底部对齐。删除确认移至操作栏上方，展开时按钮仍贴底。
- 新增桌面/手机浏览器回归，以短文件名和换行的长文件名验证网格同行按钮对齐、操作栏距卡片底部的间距、列表模式及删除确认状态，并保存两种布局截图。

已验证：

- `bun run typecheck`、`git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '素材卡片操作|素材库上传'` **4/4 通过**；已查看桌面和手机网格、手机列表截图，操作栏贴底且无横向溢出。

待完成 / 边界：

- 本轮仅调整素材卡片布局；未执行全量回归或远程部署。

## 本轮：素材引用事项改为点击计数查看（2026-09-25）

已编写：

- 素材库网格和列表移除常驻引用事项区域，将「N 个事项引用」改为按钮；0 条时禁用。有引用时用 Nuxt UI `UPopover` 在计数旁显示事项标题、项目及日期，长列表在小窗内滚动，portal 避免素材卡片裁剪；点击事项关闭小窗并沿用总览定位（包括已完成、无日期），跳转时避免小窗关闭抢回焦点。点击外部或 Escape 关闭。README 更新使用说明。
- 扩展现有桌面/手机素材库浏览器回归：0 条不可点击、默认不显示事项、网格和列表点击显示、Escape/外部关闭、引用跳转、弹窗视口边界与无页面横向溢出。

已验证：

- `bun run typecheck`、`git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '素材库上传'` **2/2 通过**，含真实本地 Worker/D1/R2 和生产构建。已查看桌面/手机 `asset-reference-popover.png` 及手机 `asset-library-list.png`：小窗定位、引用条目与紧凑列表排布正常。

待完成 / 边界：

- 本轮仅调整前端引用事项的展示入口；未运行全量回归或远程部署。远程资源与 Images binding 的上线要求仍见下节。
...
- 编辑事项底栏左侧改为浅色紧凑图标组「完成状态 / 导出 / 删除」，右侧为无边框「取消」及粉色「保存」；手机窄屏优先同排。新增空心圆 / 绿色圆圈勾选图标区分未完成与已完成，保存按钮去掉重复的对勾；新建事项也沿用完成状态图标。
- 完成状态按钮保留表单草稿切换语义，提供动态操作说明、`aria-pressed`、键盘操作；更新桌面/手机浏览器回归，检查按钮顺序、状态往返、保存流程、320px 同排和无横向溢出。

已验证：

- `bun run typecheck`、`git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '单事项卡片与编辑弹窗打开独立打印页|邮箱进入、项目和事项编辑'` **4/4 通过**；截图去掉测试键盘焦点轮廓后打印专项 **2/2 通过**。320px 尺寸变化时用同次布局读取检查左右操作组同排，避免长描述重新测高引起的短暂位移误判。
- 已查看桌面和手机 `entry-editor-actions.png`：图标组与右侧文字操作同排，完成状态区分清楚。

待完成 / 边界：

- 本轮未重跑全量回归或部署。

## 本轮：导出按钮移至删除旁（2026-09-24）

已编写：

- 编辑事项底栏将仅图标的导出按钮移到删除按钮右侧；完成状态紧随其后，取消与保存仍在右侧。更新既有打印回归，断言按钮顺序。

已验证：

- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '单事项卡片与编辑弹窗打开独立打印页'` **2/2 通过**；首轮新增顺序断言的定位匹配两个按钮，限定首个按钮后复验通过。手机底栏截图已查看，布局正常。
- `git diff --check` 通过。

待完成 / 边界：

- 本次仅调整底栏顺序，未重跑全量回归或部署。

## 本轮：精简事项编辑与引用创建日期（2026-09-24）

已编写：

- 事项编辑弹窗导出按钮改为仅图标（保留可访问名称及悬浮标题），「保存修改」改为「保存」；所属项目改用 Nuxt UI `USelect`，下拉留在弹窗内，按 Escape 只关闭菜单而不关闭编辑弹窗。
- 描述输入时的 @ 引用候选显示目标事项的本地添加日期；共用事项悬浮预览也显示带完整时间戳的添加日期，因此描述中已插入的引用悬浮详情可查看创建日期。
- 更新桌面和手机浏览器回归：下拉顺序、选择、Escape、按钮文案、候选添加日期和描述引用悬浮日期；底栏截图滚动至按钮区域。

已验证：

- `bun run typecheck`、`git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '单事项卡片与编辑弹窗打开独立打印页|统一项目排序弹窗|邮箱进入、项目和事项编辑|描述 Markdown 安全展示|长描述详情|项目总览分组'` **12/12 通过**。首次运行发现项目下拉 Escape 同时关闭弹窗，修正后专项 2/2、以上整组 12/12 通过；之后仅调整模板排版及底栏截图滚动位置，打印专项再验证 **2/2 通过**。
- 已查看桌面与手机 `entry-editor-actions.png` 及手机 `description-picker.png`：按钮、下拉触发器、创建日期与窄屏布局正常。

待完成 / 边界：

- 未重跑全量 API / 浏览器回归，未部署。

## 本轮：月／周视图拖动事项改期（2026-09-24）

已编写：

- 月、周事项标签共用 Pointer Events 拖动逻辑：拖动预览随指针移动，目标日期高亮；在原日期或日历外松开、Escape 取消不保存。触摸拖动与桌面点击编辑、悬浮详情及手机周视图点按详情共存；保存中禁用拖动。月视图支持当前 42 格（含相邻月补位），周视图支持当前七天。
- 手机月视图显示每天最多三个事项标签，日历区域横向滚动，进入月视图时将选中日期滚到可见位置，拖到边缘可继续横向滚动；保留每日数量和超出三个事项的入口。
- API `PATCH /api/entries/:id` 增加仅更新 `date` 的分支，与原完成状态分支互斥，共用日期校验及 Drizzle 更新；拖动保存通过该分支，仅改日期而不覆盖描述、完成状态、引用等字段。失败保留原日期并显示错误。README 更新交互与接口说明；API、桌面和手机回归覆盖跨月/跨年、点按、取消、失败、刷新持久化及无横向溢出。

已验证：

- `bun run typecheck`（Web、API、测试）和 `git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test` **40 通过、1 桌面专属跳过**（41 项）：包含新增改期 API 回归、桌面/手机真实 D1 拖动回归与既有功能。之后补充拖动预览及手机月视图自动显示所选日期，专项 `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '月周事项标签拖动|邮箱进入、项目和事项编辑|项目总览分组'` **6/6 通过**，并再次通过类型检查与差异检查。
- 最后补充月视图标签点按回归、拖动过程页面无横向溢出断言及预览位置边界后，`CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '月周事项标签拖动'` **2/2 通过**。
- 已查看两端 `calendar-entry-drag.png`：来源标签淡化、拖动预览及目标日期高亮可见；手机月视图标签与横向滚动布局正常。

待完成 / 边界：

- 补充预览和自动显示选中日期后未再次运行全量回归；未验证实体手机或非 Chromium 浏览器，未部署。
...
- 将编辑事项底栏分成「删除 / 已完成」和「导出 / 取消 / 保存」两组；导出改为带文字的次级按钮。窄屏下两组分行，保持操作含义和视觉间距清晰。
- 单事项浏览器回归增加编辑弹窗底栏截图及手机无横向溢出检查。

已验证：

- `bun run --filter @agenda/web typecheck`、`git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '单事项卡片与编辑弹窗打开独立打印页|邮箱进入、项目和事项编辑'` **4/4 通过**；新增截图后打印专项复验 **2/2 通过**。已查看桌面与手机 `entry-editor-actions.png`，操作分组与窄屏换行正常。

待完成 / 边界：

- 本轮未重新执行全量浏览器回归或部署。

## 本轮：单事项打印 / PDF 导出（2026-09-23）

已编写：

- 日历详情与项目总览的事项卡片增加打印图标；已有事项的编辑弹窗增加「导出已保存事项」入口。点击后同步打开新标签页，不包含编辑弹窗内尚未保存的修改。
- 打印页根据 URL 中的事项 ID、浏览器记住的邮箱重新获取当前空间数据；不在 URL 中传邮箱。显示项目、日期、完成状态、本地添加日期、完整 Markdown 描述、旧版独立引用和反向引用。邮箱未记住时可在打印页输入；加载失败可重试，已删除或其他邮箱的事项显示找不到。
- 独立预览提供「打印 / 保存为 PDF」，A4 纸张样式隐藏操作栏、保留长描述自然分页；README 更新使用说明。批量事项导出仍搁置。

已验证：

- `bun run typecheck`、`git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '单事项卡片与编辑弹窗打开独立打印页'` **2/2 通过**：验证两处卡片入口与弹窗入口、未保存修改、无日期与已完成事项、跨邮箱不可见、刷新、新窗口、打印媒体、浏览器 PDF 输出及长描述。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '项目总览分组|邮箱进入、项目和事项编辑'` **4/4 通过**。已查看桌面/手机打印页截图，排版正常、无横向溢出。

待完成 / 边界：

- 本轮未运行全量浏览器回归，未在实体设备或非 Chromium 浏览器验证打印对话框，未部署。浏览器 PDF 由 Playwright 验证生成，用户打印机或系统「存储为 PDF」界面仍取决于浏览器。

## 本轮：统一项目排序入口与排序弹窗（2026-09-23）

已编写：

- 项目栏移除每个项目左侧常驻拖动手柄，在「我的项目」旁新增统一排序图标按钮。点击打开独立排序弹窗，按当前项目顺序上下拖动手柄或聚焦后按上下方向键调整；取消不保存，确认后一次提交。原项目「…」菜单的上移 / 下移保留。
- 弹窗沿用既有排序 API、保存与刷新流程；保存期间禁用关闭和重复提交，失败保留弹窗草稿并显示错误，侧栏继续保留云端原顺序。移动端弹窗纵向拖动，不再依赖侧栏横向拖动；README 同步操作说明。
- 更新桌面/手机浏览器回归，覆盖菜单排序、统一入口、取消、键盘、拖动、保存等待与失败、刷新持久化、总览及事项表单顺序、无横向溢出。

已验证：

- `bun run --filter @agenda/web typecheck`、`bun run typecheck:tests`、`git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '统一项目排序弹窗|项目总览分组'` **4/4 通过**；补充弹窗保存等待断言后，排序专项复验 **2/2 通过**。
- 已查看桌面/手机 `project-sort-editor.png`，项目栏入口与弹窗布局正常。

待完成 / 边界：

- 未重跑全量浏览器回归或验证实体设备，未部署。与本轮功能无关的 `opencode.json` 权限配置修改未由本轮编辑，按「全部提交」要求一并纳入提交。
...
- 日历新增「周」视图，周一至周日纵向七行，每行事项标题横向排列并自动换行；选中日期和今天沿用粉色强调，下方保留单日事项详情。可按周前后导航、选择日期跳周、回到今天；标题与统计按当前周显示，跨月/跨年使用本地日历日期。
- 桌面首次打开及刷新默认月视图，宽度不超过 600px 的移动端默认周视图；手动视图选择在工作台日历/总览切换时保留，刷新恢复设备默认。周视图的桌面事项标题悬浮/键盘聚焦显示详情、点击可编辑；无悬浮能力的手机点击标题打开详情浮层，选中日期后从下方单日详情编辑。提取 `EntryPreview.vue` 共用原 tooltip 与手机 popover 内容。
- README 更新操作方式；现有手机月视图流程显式切换到月，新增桌面/手机周视图浏览器回归，验证七日范围、跨年导航、周统计、项目筛选、日期添加、展示和编辑、视图切换、刷新默认及无横向溢出。

已验证：

- `bun run --filter @agenda/web typecheck`、`bun run typecheck:tests`、`git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile` **20/20 通过**。初次运行手机旧主流程停在「下个月」，原因是刷新后按新要求恢复手机周视图；为原月视图回归显式切换「月」后，专项及全量复验通过。
- 已查看两端 `week-view.png`：纵向七行、事项标签横排及下方日期详情正常；新测试检查两端无页面横向溢出。

待完成 / 边界：

- 本轮已提交；未执行 API 全量回归或部署。手机交互仅在 Chromium 模拟设备验证，未验证实体设备。
...
- 修正前轮仅过滤整个项目分组的实现：关闭「显示已完成」时，各项目内的已完成事项也隐藏；全部完成的分组继续隐藏，空项目保留，统计仍使用完整事项数据。
- 引用跳转到任意已完成事项时自动开启显示，不再要求目标项目全部完成。
- 扩展总览回归，覆盖混合分组的默认隐藏、开关往返、标记完成后即时隐藏、恢复未完成，以及混合分组内已完成引用目标的展开和聚焦。

已验证：

- `bun run --filter @agenda/web typecheck`、`bun run typecheck:tests`、`git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '项目总览分组'` **2/2 通过**。

待完成 / 边界：

- 本轮未重跑全量测试，未提交或部署。

## 本轮：精简总览操作（2026-09-21）

已编写：

- 总览开关文案精简为「显示已完成」，移除总览「清除项目筛选」按钮及对应事件绑定。同步现有测试文案，测试通过侧栏「全部项目」清除筛选。
...
## 本轮：总览完成项目开关（2026-09-21）

已编写：

- `ProjectOverview.vue` 的「清除项目筛选」改为 Nuxt UI `UButton`，新增 `USwitch` 布尔开关「显示已完成项目」，默认关闭。有事项且全部完成的项目隐藏，空项目及含未完成事项的项目保留；统计仍包含当前项目筛选下的全部事项。全部被隐藏时显示对应提示。
- 开关状态由 `app.vue` 保存，切换视图保留，刷新或进入邮箱时重置。引用跳转到已完成项目会自动打开开关并保持目标聚焦；工具栏操作支持换行。
- 扩展现有总览回归，覆盖清除筛选、完成最后一个事项后隐藏、开关显示/隐藏、刷新重置及引用自动展开。

已验证：

- `bun run --filter @agenda/web typecheck`、`bun run typecheck:tests`、`git diff --check` 通过。
- `CHOKIDAR_USEPOLLING=1 bun run test --project=desktop --project=mobile -g '项目总览分组'` **2/2 通过**，包含两端无横向溢出断言。
