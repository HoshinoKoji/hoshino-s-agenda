...
const props = defineProps<{ start: Date; selected: string; today: string; entries: Entry[]; allEntries: Entry[]; projects: Project[] }>()
...
<template>
  <div class="calendar-week" role="group" aria-label="周日历">
    <div v-for="day in days" :key="day.key" class="week-row" :class="{ 'is-selected': day.key === selected, 'is-today': day.key === today, weekend: day.weekend }" :data-date="day.key">
      ...
      <div class="week-entries">
        <template v-if="day.entries.length">
          <EntryTooltip v-for="entry in day.entries" :key="entry.id" :entry="entry" :entries="allEntries" :projects="projects" :tap="touch">
            <button class="week-entry" :class="{ completed: entry.completed }" :style="{ '--project-color': projectMap.get(entry.projectId)?.color || '#8574D8' }" :aria-label="`${entry.title}${touch ? '，查看详情' : '，编辑事项'}`" @click="touch ? undefined : emit('edit', entry)"><AppIcon v-if="entry.completed" name="check" :size="13" /><i v-else class="entry-tick" /><span>{{ entry.title }}</span></button>
          </EntryTooltip>
        </template>
