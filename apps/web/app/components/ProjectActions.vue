...
const emit = defineEmits<{ overview: []; edit: [] }>()
...
<template>
  ...
  >
    ...
    <template #item-leading="{ item }"><AppIcon :name="item.action === 'overview' ? 'grid' : 'edit'" :size="16" class="text-[#98919f] group-data-highlighted:text-primary" /></template>
  </UDropdownMenu>
</template>
