...
const groups = computed(() => {
  ...
  return props.projects.filter(project => !props.activeProject || project.id === props.activeProject).map(project => {
    ...
    return { project, entries, remaining: entries.filter(entry => !entry.completed).length }
  })
})
...
<template>
  <section class="project-overview" aria-label="项目总览" :aria-busy="loading">
    <header class="overview-toolbar">
      ...
      <div class="overview-actions"><button v-if="activeProject" class="text-button" @click="emit('clearFilter')">清除项目筛选</button><button class="button secondary" :disabled="busy" @click="emit('createProject')"><AppIcon name="plus" :size="16" />项目</button></div>
    </header>
    ...
    <section v-for="group in groups" :key="group.project.id" class="overview-project" :aria-labelledby="`project-title-${group.project.id}`">
      ...
      <EntryList v-if="group.entries.length" :entries="group.entries" :all-entries="entries" :projects="projects" :busy="busy" show-date @edit="emit('edit', $event)" @toggle="emit('toggle', $event)" @follow="emit('follow', $event)" />
