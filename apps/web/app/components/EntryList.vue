  ...
  <div class="entry-list">
    <article v-for="entry in entries" :id="`entry-${entry.id}`" :key="entry.id" class="entry-card" :class="{ 'entry-completed': entry.completed }" tabindex="-1" :aria-label="entry.title">
      ...
      <div class="entry-content">
        ...
        <div class="entry-meta"><span class="project-tag" :style="{ '--project-color': projectMap.get(entry.projectId)?.color }"><i class="project-dot" />{{ projectMap.get(entry.projectId)?.name }}</span><span v-if="showDate" class="entry-state">{{ entry.date ?? '未设日期' }}</span><span class="entry-state">{{ entry.completed ? '已完成' : '进行中' }}</span></div>
        <EntryDescription :entry="entry" :entries="allEntries" :projects="projects" @follow="emit('follow', $event)" />
