...
watch([descriptionElement, () => props.entry.description, () => props.collapsible], ([element], _, onCleanup) => {
...
<template>
  <div v-if="entry.description" :id="descriptionId" ref="descriptionElement" class="entry-description" :class="{ 'is-collapsed': collapsible && !expanded }" @focusin="expanded = collapsible || expanded"><EntryMarkdown :entry="entry" :entries="entries" :projects="projects" :interactive="interactive" @follow="emit('follow', $event)" /></div>
