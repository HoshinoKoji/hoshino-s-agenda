...
test('流式日视图支持日期导航、筛选、编辑和跨月引用', async ({ page, space }, testInfo) => {
  ...
  await expect(page.getByLabel('日视图日期')).toHaveValue('2026-08-31')
  ...
  await expect(page.getByLabel('日视图日期')).toHaveValue('2026-09-01')
  await expect(page.locator('.day-empty')).toContainText('当天暂无事项')
  await page.locator('.add-main').click()
  await expect(page.getByLabel('记录日期')).toHaveValue('2026-09-01')
  ...
  await expect(page.getByLabel('日视图日期')).toHaveValue('2026-08-31')
  await page.getByLabel('日视图日期').fill('2024-02-28')
  await page.getByRole('button', { name: '后一天', exact: true }).click()
  await expect(page.getByLabel('日视图日期')).toHaveValue('2024-02-29')
  await page.getByRole('button', { name: '今天', exact: true }).click()
  await expect(page.getByLabel('日视图日期')).toHaveValue('2026-09-13')
  await noOverflow(page)
