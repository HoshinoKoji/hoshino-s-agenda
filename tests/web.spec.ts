...
test('素材库上传、事项引用与上传附件、图片预览和打印', async ({ page, space, otherSpace, isMobile }, testInfo) => {
  ...
  await page.getByRole('button', { name: '下载素材 笔记.txt' }).click()
  expect((await libraryDownload).suggestedFilename()).toBe('笔记.txt')
  ...
  await page.getByRole('button', { name: '移除附件 笔记.txt' }).click()
  await page.getByRole('button', { name: '保存', exact: true }).click()
  await switchView('素材库')
