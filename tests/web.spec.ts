...
test('素材库上传、事项引用与上传附件、图片预览和打印', async ({ page, space, otherSpace, isMobile }, testInfo) => {
  ...
  const id = await space.entry(project.id, '有附件的事项', null)
  const png = samplePng()
  const imageRequests: string[] = []
  page.on('request', request => { if (request.url().includes('/api/assets/')) imageRequests.push(request.url()) })
  ...
  await switchView('素材库')
  await page.getByLabel('上传素材').setInputFiles({ name: '图像.png', mimeType: 'image/png', buffer: png })
  await expect(page.getByRole('article').filter({ hasText: '图像.png' })).toBeVisible()
  const pictureCard = page.locator('.asset-library-card').filter({ hasText: '图像.png' })
  await expect(pictureCard).not.toContainText('image/png')
  await expect(pictureCard.getByRole('button', { name: '图像.png · 0 个事项引用' })).toBeDisabled()
  await expect.poll(() => imageRequests.some(url => url.endsWith('/thumbnail'))).toBe(true)
  await expect(pictureCard.locator('img')).toHaveJSProperty('naturalWidth', 320)
  await page.getByRole('button', { name: '图片', exact: true }).click()
  await expect(page.locator('.asset-library-card')).toHaveCount(1)
  await page.getByRole('button', { name: '预览图片 图像.png' }).click()
  await expect(page.getByRole('dialog', { name: '图像.png' }).locator('img')).toBeVisible()
  await expect.poll(() => imageRequests.some(url => url.endsWith('/content'))).toBe(true)
  await page.getByRole('dialog', { name: '图像.png' }).getByRole('button', { name: '关闭弹窗' }).click()
  await switchView('项目总览')
  await page.locator(`#entry-${id}`).getByRole('button', { name: '编辑事项 有附件的事项' }).click()
  await page.getByRole('button', { name: '从素材库选择' }).click()
  await page.getByRole('button', { name: '图像.png', exact: true }).click()
  await page.getByLabel('上传事项附件').setInputFiles({ name: '笔记.txt', mimeType: 'text/plain', buffer: Buffer.from('asset note') })
  await expect(page.getByRole('button', { name: '移除附件 笔记.txt' })).toBeVisible()
  await page.getByRole('button', { name: '保存', exact: true }).click()
  await expect(page.getByRole('dialog')).toBeHidden()
  const card = page.locator(`#entry-${id}`)
  await expect(card.getByRole('region', { name: '事项附件' })).toContainText('图像.png')
  await expect(card.getByRole('region', { name: '事项附件' })).toContainText('笔记.txt')
  ...
  await expect(printPage.getByRole('region', { name: '事项附件' })).toContainText('图像.png')
  await expect(printPage.locator('.asset-print-image')).toBeVisible()
  await expect.poll(() => printPage.locator('.asset-print-image').evaluate(image => (image as HTMLImageElement).naturalWidth)).toBeGreaterThan(0)
  await printPage.emulateMedia({ media: 'print' })
  expect((await printPage.pdf({ format: 'A4', printBackground: true })).subarray(0, 5).toString()).toBe('%PDF-')
  await printPage.close()
  await switchView('素材库')
  await expect(page.getByRole('button', { name: '删除素材 图像.png' })).toBeDisabled()
  const pictureReferences = pictureCard.getByRole('button', { name: '图像.png · 1 个事项引用' })
  const referencePopover = page.locator('.asset-reference-popover')
  await expect(pictureCard.getByRole('button', { name: '转到事项 有附件的事项' })).toHaveCount(0)
  await pictureReferences.click()
  await expect(referencePopover.getByRole('button', { name: '转到事项 有附件的事项' })).toContainText('素材项目 · 未设日期')
  await page.keyboard.press('Escape')
  await expect(referencePopover).toBeHidden()
  await expect(pictureReferences).toBeFocused()
  await page.getByRole('button', { name: '列表视图' }).click()
  await expect(page.getByRole('button', { name: '列表视图' })).toHaveAttribute('aria-pressed', 'true')
  await expect(page.locator('.asset-library-list .asset-library-card')).toHaveCount(2)
  await page.getByRole('button', { name: '文件', exact: true }).click()
  await expect(page.locator('.asset-library-card')).toHaveCount(1)
  await page.getByRole('searchbox', { name: '搜索素材' }).fill('笔记')
  await page.getByRole('button', { name: '重命名素材 笔记.txt' }).click()
  await page.getByRole('textbox', { name: '新素材名称 笔记.txt' }).fill('新笔记.txt')
  await page.locator('.asset-rename-form').getByRole('button', { name: '保存' }).click()
  await expect(page.getByRole('button', { name: '下载素材 新笔记.txt' })).toBeVisible()
  await expect(page.locator('.asset-library-card')).not.toContainText('text/plain')
  await page.getByRole('button', { name: '新笔记.txt · 1 个事项引用' }).click()
  await expect(referencePopover.getByRole('button', { name: '转到事项 有附件的事项' })).toBeVisible()
  await page.getByRole('searchbox', { name: '搜索素材' }).click()
  await expect(referencePopover).toBeHidden()
  ...
  await page.screenshot({ path: testInfo.outputPath('asset-library-list.png'), animations: 'disabled' })
  await page.getByRole('button', { name: '网格视图' }).click()
  await expect(page.locator('.asset-library-grid:not(.asset-library-list) .asset-library-card')).toHaveCount(2)
  await page.screenshot({ path: testInfo.outputPath('asset-library.png'), animations: 'disabled' })
  await pictureReferences.click()
  const popoverBounds = await referencePopover.boundingBox()
  ...
  await noOverflow(page)
  await page.screenshot({ path: testInfo.outputPath('asset-reference-popover.png'), animations: 'disabled' })
  await referencePopover.getByRole('button', { name: '转到事项 有附件的事项' }).click()
  await expect(view).toContainText('项目总览')
  await expect(card).toBeFocused()
  await expect(card.getByRole('region', { name: '事项附件' })).toContainText('新笔记.txt')
  await card.getByRole('button', { name: '标为完成：有附件的事项' }).click()
  ...
  await expect(card).toBeFocused()
  await card.getByRole('button', { name: '编辑事项 有附件的事项' }).click()
  await page.getByRole('button', { name: '移除附件 图像.png' }).click()
  await page.getByRole('button', { name: '移除附件 新笔记.txt' }).click()
  await page.getByRole('button', { name: '保存', exact: true }).click()
  await switchView('素材库')
  await page.getByRole('button', { name: '全部', exact: true }).click()
  await expect(page.getByRole('button', { name: '删除素材 图像.png' })).toBeEnabled()
  await page.getByRole('button', { name: '删除素材 图像.png' }).click()
...
test('单事项卡片与编辑弹窗打开独立打印页', async ({ page, space, otherSpace, isMobile }, testInfo) => {
  ...
  const completion = page.locator('.entry-footer-settings .completion-field')
  await expect(completion).toHaveAttribute('aria-pressed', 'true')
  await expect(completion).toHaveAttribute('title', '已完成 · 点击标为未完成')
  await completion.click()
  await expect(completion).toHaveAttribute('aria-pressed', 'false')
  await expect(completion).toHaveAttribute('title', '未完成 · 点击标为完成')
  await completion.focus()
  await page.keyboard.press('Space')
  await expect(completion).toHaveAttribute('aria-pressed', 'true')
  await expect(page.locator('.entry-footer-settings > button')).toHaveCount(3)
  await expect(page.locator('.entry-footer-settings > button:nth-child(2)')).toHaveAttribute('aria-label', '导出已保存事项')
  await expect(page.locator('.entry-footer-settings > button:nth-child(3)')).toHaveAttribute('aria-label', '删除事项')
  await expect(page.getByRole('button', { name: '导出已保存事项' })).toHaveText('')
  await expect(page.getByRole('button', { name: '保存', exact: true })).toHaveText('保存')
  await noOverflow(page)
  await completion.evaluate(element => (element as HTMLButtonElement).blur())
  await page.locator('.entry-form-footer').scrollIntoViewIfNeeded()
  await page.screenshot({ path: testInfo.outputPath('entry-editor-actions.png'), animations: 'disabled' })
  if (isMobile) {
    await page.setViewportSize({ width: 320, height: 844 })
    await noOverflow(page)
    await expect.poll(() => page.locator('.entry-form-footer').evaluate(element =>
      Math.abs(element.children[0]!.getBoundingClientRect().y - element.children[1]!.getBoundingClientRect().y))).toBeLessThan(5)
...
test('统一项目排序弹窗、菜单、刷新、视图同步与保存失败', async ({ page, space, isMobile }, testInfo) => {
  ...
  await page.getByRole('combobox', { name: '所属项目' }).click()
  await expect(page.getByRole('option')).toHaveText(['甲', '丙', '乙'])
  await page.keyboard.press('Escape')
  await page.getByRole('button', { name: '取消', exact: true }).click()
...
test('邮箱进入、项目和事项编辑、完成、双向引用跳转与删除', async ({ page, space, otherSpace, baseURL, isMobile }, testInfo) => {
  ...
  await page.getByRole('combobox', { name: '所属项目' }).click()
  await page.getByRole('option', { name: '网站计划' }).click()
  ...
  await page.getByRole('combobox', { name: '所属项目' }).click()
  await page.getByRole('option', { name: '阅读计划' }).click()
  ...
  await page.getByRole('button', { name: '标为未完成', exact: true }).click()
  await expect(page.getByRole('button', { name: '标为完成', exact: true })).toHaveAttribute('aria-pressed', 'false')
...
test('项目总览分组、无日期事项管理、筛选保留与跨视图引用', async ({ page, space, isMobile }, testInfo) => {
  ...
  await expect(page.getByRole('combobox', { name: '所属项目' })).toHaveText(empty.name)
...
test('描述 Markdown 安全展示、@筛选与键盘中间插入、刷新及引用删除同步', async ({ page, space, isMobile }, testInfo) => {
  await page.clock.setFixedTime(new Date('2026-09-13T04:00:00Z'))
  ...
  const createdAt = (await space.agenda()).entries.find(entry => entry.id === first)!.createdAt
  const createdDate = await page.evaluate(value => {
    const date = new Date(value)
    return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`
  }, createdAt)
  await expect(options).toContainText([/添加于 \d{4}-\d{2}-\d{2}/, `添加于 ${createdDate}`])
  ...
  if (!isMobile) {
    await rendered.locator('.entry-mention').hover()
    const referencePreview = page.locator('.calendar-tooltip').filter({ hasText: '相同标题' })
    await expect(referencePreview).toBeVisible()
    await expect(referencePreview.locator('time')).toHaveAttribute('datetime', (await space.agenda()).entries.find(entry => entry.id === second)!.createdAt)
    await expect(referencePreview.locator('time')).toContainText('添加于 ')
  }
...
test('长描述详情不溢出，桌面 tooltip 悬停/键盘聚焦及 viewport 边界', async ({ page, space, isMobile }, testInfo) => {
  ...
  await expect(tooltip.locator('time')).toContainText('添加于 ')
  await expect(tooltip.locator('time')).toHaveAttribute('datetime', /.+/)
